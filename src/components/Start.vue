    ...
    <div class="content-container">
      ...
      <div v-else-if="currentStep === 'start'" class="start-survey-container">
        <h2>
          ...
          connaitre vos habitudes de déplacement sur ce secteur<br />
          accepteriez-vous de répondre à 4 questions ?
        </h2>
        <h2></h2>
...
const progress = computed(() => {
  ...
  const isLastOrEnding =
    ...
    currentQuestion.value?.options?.some(
      (option) => option.next === "end" || option.next === "fin"
    ) ||
    currentQuestion.value?.next === "end" ||
    currentQuestion.value?.next === "fin";
...
const nextQuestion = (forcedNextId = null) => {
  ...
  if (nextQuestionId === "end" || nextQuestionId === "fin") {
    finishSurvey();
...
const getNextId = async () => {
  ...
  return `TrappesVoiture-${counter.toString().padStart(6, "0")}`;
};
</script>
